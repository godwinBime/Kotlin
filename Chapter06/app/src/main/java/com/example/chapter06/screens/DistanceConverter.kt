...
import com.example.chapter06.viewmodels.DistancesViewModel

@Composable
fun DistanceConverter (viewModel: DistancesViewModel) {
    ...
    Column(modifier = Modifier
        ...
        DistanceScaleButtonGroup(selected = unit, modifier = Modifier.padding(bottom = 16.dp) ){
            resId: Int -> viewModel.setUnit(resId)
        }
        Button(onClick = calc, enabled = enabled) {
...
fun DistanceTextField(
    ...
    TextField(value = distance.value, onValueChange = {viewModel.setDistance(it)},
        placeholder = {
